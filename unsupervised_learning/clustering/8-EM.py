...
def expectation_maximization(X, k, iterations=1000, tol=1e-5, verbose=False,
    ...
    tol=1e-5, verbose=False): that performs the expectation maximization for a
    GMM:
    ...
    try:
        ...
        if verbose:
            print('Log Likelihood after {} iterations: {:.5f}'.format(0,
                                                                      prev_l))
            ...
            if np.abs(current_l - prev_l) <= tol:
                if verbose:
                    print('Log Likelihood after' +
                          ' {} iterations: {:.5f}'.format(i, current_l))
